...
class LoadingManager:
    ...
    def __init__(self):
        ...
        for event_type, handler in incoming.items():
            self._subscriptions.append(message_bus.create_subscription_to_pop(handler, name=event_type))

        # -- subscribe to stage events
        event_stream = omni.usd.get_context().get_stage_event_stream()
        self._subscriptions.append(event_stream.create_subscription_to_pop(self._on_stage_event))
    ...
    def _on_open_stage(self, event: carb.events.IEvent) -> None:
        ...
        if event.type == carb.events.type_from_string("openStageRequest"):
            ...
            if "url" not in event.payload:
                carb.log_error(f"Unexpected message payload: missing \"url\" key. Payload: '{event.payload}'")
                return
