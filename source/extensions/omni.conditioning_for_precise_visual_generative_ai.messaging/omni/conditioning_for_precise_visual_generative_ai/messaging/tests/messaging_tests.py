...
class MessagingTest(AsyncTestCase):
    async def setUp(self):
        ...
        extension = "omni.conditioning_for_precise_visual_generative_ai.messaging"
    ...
    async def test_stage_loading_incoming(self):
        ...
        for event in outgoing.keys():
            subscriptions.append(self._message_bus.create_subscription_to_pop(on_message_event, name=event))
    ...
    async def test_stage_management_incoming(self):
        ...
        incoming: List[str] = ["getChildrenRequest", "selectPrimsRequest", "makePrimsPickable", "resetStage"]
